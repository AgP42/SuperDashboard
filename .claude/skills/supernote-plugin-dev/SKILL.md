...
| Pre-submission self-audit for the plugin store (permissions, deletes, network, description) | `references/publish-review.md` |
...
35. **`PluginManager.addPluginLifeListener` was removed (0.1.65)**: the old `addPluginLifeListener({onStart, onStop})` is `undefined` at runtime (and TS rejects it). Use `registerPluginLifeListener({onMsg(msg)})`; `msg.state` carries the lifecycle value (0..5), and state 5 = destroy/uninstall. See Pattern 9.
36. **`PluginDocAPI.generateDocImage` → `generateCurrentDocImage` (0.1.65)**: the new call always targets the currently-open document and adds a required `type` param (0 = default, 1 = with text-selection styling). You can no longer render an arbitrary closed document's page via this API.
37. **`PointUtils.getNotePageSize` was removed (0.1.65)**: get page size from `PluginFileAPI.getPageSize(filePath, page)` (any file) or `PluginCommAPI.getPageDisplaySize()` (current file only). For the page-element CRUD APIs use `getPageDisplaySize` — `getPageSize` there can misalign coordinates.
38. **Permission-gated APIs fail *silently* (no thrown error) when the permission isn't declared**: on Chauvet 3.29.43+, `PluginFileAPI.getPageSize` (and other shared-storage reads) became `FILE:READ`-gated. An undeclared plugin gets `PluginSec: DENY reason=sdcard_no_read ...` in `adb logcat`, and the call resolves as if the result were unavailable — no catchable JS error, easy to misdiagnose as a lasso/geometry bug. Prefer `PluginCommAPI.getPageDisplaySize()` (current-page context, ungated, no `filePath`) when you only need the open file's size; declare `FILE:READ` (Pattern 17) only when you need an arbitrary file. **When any previously-working `PluginFileAPI`/network call starts silently no-opping after a firmware/SDK update, grep `adb logcat` for `PluginSec.*DENY` before assuming your own logic broke.** (Same class as `/Recent` reads and any raw `java.io` access outside the plugin's private dir — those throw `AccessDeniedException` without `FILE:READ`.)
39. **Reinstalling from Settings → Apps → Plugins does NOT read what you just `adb push`ed to `MyStyle/` root**: the host installs from its own managed copy at `MyStyle/Plugins/<name>.snplg`. Pushing a rebuilt `.snplg` to `MyStyle/` and tapping the existing entry's reinstall silently reruns the **old** build (same version) — looks like "the fix did nothing." Push directly to `MyStyle/Plugins/<name>.snplg` (overwrite in place), or use **Choose Installation Package** / **Add Plugin** to browse to the new file explicitly.
40. **`__DEV__`-gated logs are invisible on a real install — `buildPlugin.sh` always bundles `--dev false`** (local test builds included, not just CI). Keep one deliberately **ungated** startup log (e.g. `${TAG} v${versionName} (code ${versionCode}) starting`, read from `PluginConfig.json`) so `adb logcat` confirms *which* build is running before you debug — this also catches gotcha #39 (stale reinstall) immediately. Route JS + native logs through one custom tag rather than the generic `ReactNativeJS`.
41. **Testing a plugin's own TCP sockets doesn't need the device on your Wi-Fi**: `adb forward tcp:<local> tcp:<device-port>` tunnels over the existing USB/ADB link (e.g. `adb forward tcp:18888 tcp:8888` then `curl http://127.0.0.1:18888/`). Combine with a single-tag `adb logcat` (#40) to tell a real relay bug from an undeclared-`INTERNET`-permission block.
42. **A hand-written `__mocks__/sn-plugin-lib.js` Jest mock drifts silently**: `sn-plugin-lib` has no JS impl off-device, so the mock exposes only the methods the plugin calls, and `tsc`/`eslint` don't check its shape against real usage. Add or rename an SDK call without updating the mock and the suite stays green until a test finally imports the caller, then throws `TypeError: <method> is not a function` far from the actual drift. Treat the mock as a checklist item on any diff that changes which `sn-plugin-lib` methods are used; prune entries for methods no longer called.
43. **Do NOT `reloadFile()` after `insertImage`/`insertSticker`/`insertPageElements` — it corrupts the element geometry**: on an anisotropic-EMR device (Manta 1920×2560 → EMR 21632×16224; A5X likewise) a `reloadFile()` right after inserting a Picture re-fits it with the wrong axis scale, so the pasted image then **squashes vertically a little on every subsequent lasso move/resize**, compounding until unreadable. The reference image plugins (`embedimage`, `wolfsolver/supernote-image-insert-plugin`) just call `insertImage` and never reload, and their images move/resize cleanly. Insert, `saveCurrentNote()` if you must persist, and **stop** — no reload. (Inserted elements are already movable/deletable without a reload.)
44. **`getLastElement().picture.rect` is returned in PIXELS, not EMR** (contra the `Picture.rect` type doc): verified on Manta — a 640px-wide clip on a 1920px page reported `right-left == 640`. Use it directly for placing adjacent elements (e.g. a caption/link under a just-inserted image); only convert if the values overflow pixel space. Getting this wrong (assuming EMR and running `emrPoint2Android`) throws the position far off-page.
45. **The eraser removes strokes and text boxes, but NOT pictures/stickers/geometry**: a pasted image (`insertImage`) or sticker cannot be rubbed out with the eraser — the user must lasso-select it and Delete. This is native Supernote behaviour, not a plugin bug; document it for any paste-image feature. (If a feature genuinely needs eraser-removable output, it must insert **strokes**, the only freely-erasable *and* distortion-free-on-move element type.)
46. **`insertTextLink` page conventions differ by `linkType`, and inserted links auto-follow reordered pages**: `linkType 0` (note page, **same file**) takes a **0-based** `destPage` (same space as `openFile`/`getCurrentPageNum`); `linkType 1` (note **file**, another note) takes a **1-based** `destPage` (passing the 0-based value lands one page early). `linkType 1` **does** honor `destPage` despite the SDK comment "only valid when linkType 0". Use `linkType 0` when the target path equals the current file (a `linkType 1` link to the current file does nothing). Once inserted, the link **follows the target page across reordering/moves** because the firmware resolves `destPage`→an internal page ID at insert time — so insert while the stored page number is still correct. The page ID is NOT surfaced through the SDK: `getLastElement` on the link returns only `{style,X,Y,width,height,fontSize,linkType,destPage,destPath,category,fullText,showText,italic}` — the `destPageId`/`destFileId`/`newDestPageNum` fields exist in the native model (commented out in `Element.js`/`LassoData.js`) but are never returned. **However, the page ID IS reachable by parsing the `.note` file directly** (see gotcha #48): so a plugin CAN make even an *external* backlink (its own `openFile(path, n)`) follow a reordered or moved page — read the current PAGEID→index map at click time and resolve. (Passing `destPageId` into `insertTextLink` via the allow-unknown passthrough is unverified for cross-note page jump; the reliable path is the `.note`-parse resolve, as in `SuperDashboard/src/notepage.ts`.)
47. **A type-2 (lasso) button greys out if the selection contains any element type not in its `editDataTypes`**: the codes are 0=stroke,1=title,2=picture,3=text,4=link,**5=geometry**. Omitting `5` (a common copy-paste from examples that list `[0,1,2,3,4]`) disables the button whenever the lasso includes a shape/straight-line/ellipse — e.g. it blocks a capture whose selection contains a "clean" underline. List every type your handler tolerates, `[0,1,2,3,4,5]` for "anything".
48. **Stable page IDs are readable by parsing the `.note` file yourself** (the SDK hides them; #46). The format, confirmed on-device: magic `noteSN` at offset 0; the **last 4 bytes** are the footer block address (uint32 little-endian); a **block** is `[uint32-LE length][payload]`; the **footer** payload is a latin1 string listing `<PAGE{n}:{addr}>` (n is **1-indexed**); each **page block** contains `<PAGEID:{stableId}>` (also `<RECOGNTEXT:{addr}>`, `<FIVESTAR:>`, `<ORIENTATION:{r}>`, `<KEYWORD_...>`). Add a native ranged reader (`RandomAccessFile` → return the bytes as an ISO-8859-1/latin1 string so JS reads LE lengths with `charCodeAt` and matches markers with a regex, no base64 needed), then: read magic+size, tail→footer addr, footer block→page list, and for a target page read just that one block (**O(1)** — don't walk every page). Build a `PAGEID↔index` map to make backlinks follow reordered/moved pages; to find a page that moved to **another** note, scan recent notes (mtime-desc, capped, early-exit) for the id. Two traps: (a) reads are `FILE:READ`-gated (#38) for notes outside your private dir; (b) a failed/empty parse must be told apart from "id genuinely absent" before self-healing a stored pointer, or a transient read error can rebind to a note holding a *copy* of the page. Reference impl: `SuperDashboard/src/notepage.ts` (same technique as SmartNoteAI's `readNote`).
